...
function StoryCard({ story, lang, onClick, status }) {
  ...
  return (
    <button onClick={onClick}
      ...
      {status === 'in_progress' && (
        <div style={{ position:'absolute', top:14, ...(lang === 'ar' ? { left:14 } : { right:14 }), padding:'2px 8px', borderRadius:20, background:'rgba(251,191,36,0.12)', border:'1px solid rgba(251,191,36,0.4)', color:'#fbbf24', fontFamily:"'Noto Naskh Arabic','Space Mono',monospace", fontSize:'0.58rem', fontWeight:700, letterSpacing:0 }}>{lang === 'ar' ? '… جارٍ القراءة' : '… Reading'}</div>
      )}
