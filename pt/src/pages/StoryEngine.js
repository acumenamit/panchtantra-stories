...
export default function StoryEngine({ story }) {
  ...
  // Single audio instance for the whole story session.
  // Lives here so it survives node transitions without unmounting.
  const { isPlaying, isSupported, speak, stop } = useAudio();
  ...
  const go = (nextId, choiceText) => {
    ...
    stop(); // stop audio before transition — prevents unmount-cancel race
    setPicked(nextId);
    setFading(true);
    setTimeout(() => {
              ...
              {textDone && (
                <div style={{ marginTop:12, marginBottom:4 }}>
                  <AudioButton
                    ...
                    onToggle={() => isPlaying ? stop() : speak(t(node.text, lang), lang)}
                    lang={lang}
                    accent={accent}
                  />
                </div>
